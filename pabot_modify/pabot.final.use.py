    ...
    try:
        ...
        if suite_names:
            #_parallel_execute_modify(datasources, options, outs_dir, pabot_args,
             #                 suite_names)
            _parallel_execute_arrange(datasources, options, outs_dir, pabot_args,
                              suite_names,"single")
        ...
        for t in threads:
            while 1:
                if t.isAlive():
                    ...
                    break
        root_name=merge_pabot_report(out_dir_common,outs_dir_list)
        option_list_common=options_list_mm[0]
        option_list_common['outputdir']=root_name
        sys.exit(_report_results(root_name, pabot_args_list[0], option_list_common, start_time_string, _get_suite_root_name(suite_names_entire)))
...
def merge_pabot_report(out_dir_common,outs_dir_list):
